...
import { ManagementPluginPayload } from '../../Shared/types';
export function WrapperBooking ( apiPayload: ManagementPluginPayload ) {
  const gVariables = apiPayload.GlobalVariables;
  ...
  const sendGridKey = CryptoJS.AES.decrypt( gVariables.sendGridEncryptedKey.toString(), gVariables.decryptionKey ).toString( CryptoJS.enc.Utf8 );
  ...
  const [maxOccupancy, setMaxOccupancy] = React.useState( gVariables.maxOccupancy );
  const [maxBookingLength, setMaxBookingLength] = React.useState( gVariables.maxBookingLength );
  ...
  const [blockingSegments, setAllBlockingSegments] = React.useState( [] );
