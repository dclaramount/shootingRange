...
export function BookingEmail(){
  ...
  return(
    <div className="reservation-order-row" style={styles.container(isNormalComputer)}>
        ...
        <input onChange={(e)=> handleEmailChange(e)} value={email} type="email" name="email" maxLength={100} size={50} id="frm-reservationCalendar-orderForm-email"  pattern={emailPattern} required={true} onInvalid={e => (e.target as HTMLInputElement).setCustomValidity('Vyplňte prosím email jako format jiri.prochazka@seznam.cz')} onInput={e => (e.target as HTMLInputElement).setCustomValidity('')}/>
