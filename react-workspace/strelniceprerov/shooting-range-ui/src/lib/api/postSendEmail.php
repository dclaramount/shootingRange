...
$query                                                =   "SELECT value FROM `global_variables` WHERE name='200_postSendEmail';";
$res                                                  =   mysqli_query($mysqli, $query, MYSQLI_USE_RESULT) or die( mysqli_error($mysqli));
while ($row = mysqli_fetch_row($res)) {
  $resultMessage[$GOOD_REQUEST]                       =   $row[0];
};
$query                                                =   "SELECT value FROM `global_variables` WHERE name='500_postSendEmail';";
$res                                                  =   mysqli_query($mysqli, $query, MYSQLI_USE_RESULT) or die( mysqli_error($mysqli));
while ($row = mysqli_fetch_row($res)) {
...
try {
  ...
  /*------------------------------------------------------------------------------*/
  send_remote_curl("Sending email to user for reservation with UUID $uuidInvoice.",(basename($_SERVER['PHP_SELF'], '.php')));
  $curl = curl_init();
  ...
  send_remote_curl('Sending Email to Customer failed.',(basename($_SERVER['PHP_SELF'], '.php')));
...
send_remote_curl("Sending Email to customer returned $requestResponse.",(basename($_SERVER['PHP_SELF'], '.php')));
