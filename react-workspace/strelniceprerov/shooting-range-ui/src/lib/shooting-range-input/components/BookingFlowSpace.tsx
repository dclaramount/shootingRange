...
import { CreatingBookingPlaceholder } from '../shared/Placeholders';
import { AdminBookingsContext } from './Context/AdminBookingsContext';
import { ConfirmationPage } from './ConfirmationPage';
import { ReservationMade } from '../../shooting-range-title/components/ReservationMade';
  ...
  React.useEffect(() =>{  
    ...
    //3.1 Verify User Entry.
      ...
      if(hasUserDataChanged(userAccount)){
        axios({
          url: `${apiURL}postUpdateUserEntry.php?id=${userAccount.id}&shootingPermit=${shootingPermit}&shootingPermitNumber=${shootingPermitNumber}&name=${name}&email=${email}&phone=${uniqueIdentifier}`,
          method: "GET",
        }).then((res) => {
    ...
    //3.2 Create New User
      ...
      axios({
        url: `${apiURL}postCreateNewUser.php?id=${userAccount.id}&shootingPermit=${shootingPermit}&shootingPermitNumber=${shootingPermitNumber}&name=${name}&email=${email}&phone=${uniqueIdentifier}`,
        method: "GET",
      }).then((res) => {
          if(res.status===200){
