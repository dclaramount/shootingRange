...
export const CopyWeekInstructorsPopUp = ({listOfAllEvents, startDateOfSelectedWeek, closeModal, toWeek} : any) => {
  const {instructorSegmentsToCopy, setInstructorSegmentsToCopy}           =   React.useContext(InstructorCalendarContext);
  ...
  const onCopyClick = () => {
    ...
    filteredSegmentsToCopy.forEach((fstc : any) => {
      ...
      const instructorId : number        = fstc.instructorId;
      const segmentToCopy : BlockSegmentToCreate = {
        ...
        url:          `start=${startDateTime}&end=${endDateTime}&guid=${uniqueGUI}&instructorId=${instructorId}`
      }
      listOfSegmentsToCopy.push(segmentToCopy);
    });
    setInstructorSegmentsToCopy(listOfSegmentsToCopy);
  ...
  // Post Creationg of Instructor Segments
  const PostBlockingCreation = (uuid : any) => {
    const segment = instructorSegmentsToCopy.find((bstc : any) => bstc.uuid===uuid.uuid);
    let     postDone                          =   false;
    const   {globalVariabes}                  =   React.useContext(ManagementDashboardContext);
    const   postCreationOfSegment             =   useGetEndPoint(globalVariabes.apiRootURL, 'postCreateInstructorSegment', segment?.url);
  ...
  return( <>
        <div style={style.wrapperPopUp()}>
            ...
            <h2 style={style.title()}>{Translations.CopyInstructorSegments.Title.toUpperCase()}</h2>
            <div style={style.wrapperTable()}>
              <table border={1}> 
                <tr> 
                  <th style={style.mainHeaderFrom()} colSpan={4}>
                    <div>
                    {`${Translations.CopyInstructorSegments.MainFromHeader.toUpperCase()} ${weekNumber}`}
                    </div>
                    <div>
                  ...
                  <th  style={style.mainHeaderFrom()} colSpan={2}>
                    <div>
                      {`${Translations.CopyInstructorSegments.MainToHeader.toUpperCase()} ${toWeekNumber}`}
                    </div>
                    <div>
                ...
                <tr> 
                  {Translations.CopyInstructorSegments.Headers.map((hf:any) => {return(<th style={style.header()}>{hf.toUpperCase()}</th> )})}
                </tr> 
                {listOfAllEvents.filter((loa:any) => (new Date(loa.start) >= startDate) && (new Date(loa.end) <= endDate)).map((event:DayPilotEvent) => {
                  ...
                  <tr id={event.uuid} style={style.normalrow()}>
                    ...
                    {instructorSegmentsToCopy.length > 0 && <th style={selectedSegments.includes(event.uuid) ? style.statusCell() : style.notSelectedStatusCell()}><PostBlockingCreation uuid={event.uuid}/></th>}
                  </tr>)
                })}
              </table>
            </div>
            <div style={style.buttonWrapper()}>
              {!copyButtonClick  && <Button  variant="contained" color="success" style={style.button()}  onClick={()=>onCopyClick()}>{Translations.CopyInstructorSegments.CopyButton}</Button>}
              {copyButtonClick  && <Button  variant="contained" color="success" style={style.closePopUpButton()}  onClick={closeModal}>{Translations.CopyInstructorSegments.CloseButton}</Button>}
            </div>
            </div>
          </>
    )
