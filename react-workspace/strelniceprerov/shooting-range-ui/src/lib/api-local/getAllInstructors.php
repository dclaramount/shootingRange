...
$query                                                =   "SELECT value FROM `global_variables` WHERE name='200_getAllInstructors';";
$res                                                  =   mysqli_query($mysqli, $query, MYSQLI_USE_RESULT) or die( mysqli_error($mysqli));
while ($row = mysqli_fetch_row($res)) {
  $resultMessage[$GOOD_REQUEST]                       =   $row[0];
}
$query                                                =   "SELECT value FROM `global_variables` WHERE name='500_getAllInstructors';";
$res                                                  =   mysqli_query($mysqli, $query, MYSQLI_USE_RESULT) or die( mysqli_error($mysqli));
while ($row = mysqli_fetch_row($res)) {
...
try{
    ...
    if ($res) {
      while ($row = mysqli_fetch_row($res)) {
        $instructorsArray[]     =array( 'name'                      => strval($row[1]),
                                        'id'                        => intval($row[0]),
                                        'color'                     => strval($row[2]),
                                        'isDeleted'                 => boolval($row[3]),
                                        'createdBy'                 => strval($row[4]),
